  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editMode) {
        ...
        setData(
          data.map((item) =>
            item.id === editId ? { ...item, ...subAssetData } : item
          )
        );
  ...
  return (
    <div className="subasset-container">
      ...
      <p className="subasset-description">
        ส่วนเพิ่มข้อมูลพัสดุย่อย สำหรับอาจารย์
        เจ้าหน้าที่ภาควิชาหรือผู้ที่มีสิทธิ์ ในการเพิ่มข้อมูลพัสดุ
      </p>
      <div className="subasset-button-container">
        <button className="subasset-button" onClick={() => handleButtonClick()}>
          + เพิ่ม
        </button>
      </div>
