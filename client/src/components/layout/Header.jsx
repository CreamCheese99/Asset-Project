...
const Header = () => {
  const { user, logout } = useUser(); // ✅ ใช้ข้อมูลผู้ใช้
    ...
    <header style={{ backgroundColor: "#8bc34a" }} className="text-white shadow-md">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between px-6 py-4">
        <div className="flex items-center space-x-4">
          <img src={logo} alt="logo" className="h-10 w-auto" />
          <div>
            <span className="text-xl font-bold">ระบบพัสดุ</span>
            <p className="text-sm">คณะครุศาสตร์อุตสาหกรรมและเทคโนโลยี</p>
          </div>
        </div>
        ...
        {user ? (
          <div className="relative group">
            <button className="flex items-center space-x-2 hover:text-gray-200">
              <UserCircle size={24} />
              <span>{user.cn}</span> {/* ✅ แสดงชื่อ */}
            </button>
            <div className="absolute right-0 mt-2 w-48 bg-white text-black shadow-lg rounded-lg p-2 hidden group-hover:block">
              <p className="p-2 text-gray-700">{user.mail}</p> {/* ✅ แสดงอีเมล */}
