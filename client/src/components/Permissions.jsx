...
const Permissions = () => {
  ...
  //   // เมื่อคลิกปุ่มแก้ไข ค่าบทบาทของผู้ใช้จะถูกตั้งค่าใน tempRole
  //   setEditingId(id);
  //   setTempRole({ ...tempRole, [id]: currentRoleId });  // ตั้งค่า role_id สำหรับผู้ใช้ที่กำลังแก้ไข
  // };

  // const handleSave = async (id) => {
  //   // ตรวจสอบการบันทึกบทบาทใหม่
  //   console.log("Saving role for user ID:", id, "New Role ID:", tempRole[id]);
  
  //   // ส่งการเปลี่ยนแปลงบทบาทไปยัง API โดยใช้ role_id ที่แก้ไขแล้ว
  //   try {
  //     const response = await axios.put(`http://localhost:5000/api/users/${id}/role`, { role_id: tempRole[id] });
  //     console.log("Response from API:", response);
  
  //     // อัปเดตข้อมูลใน state ของ users เพื่อแสดงบทบาทที่แก้ไขแล้ว
  //     setUsers(users.map(user =>
  //       user.user_id === id ? { ...user, role_id: tempRole[id], role_name: roles.find(role => role.role_id === tempRole[id]).role_name } : user
  //     ));
  
  //     setEditingId(null);  // ตั้งค่ากลับหลังจากบันทึก
  //     alert('บันทึกบทบาทผู้ใช้สำเร็จ!');
  //   } catch (err) {
  //     console.error("Error saving user role:", err);
  //     alert('ไม่สามารถบันทึกบทบาทผู้ใช้ได้');
  //   }
  // };
  ...
  return (
    <div className="container mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      {isModalOpen && (
        <div className="fixed top-0 left-0 z-10 w-full h-full bg-gray-500 bg-opacity-50 flex justify-center items-center">
