...
const ManageInfo = () => {
  ...
  //   try {
  //     if (editingId) {
  //       await axios.put(`http://localhost:5001/department/${editingId}`, tempData);
  //     } else {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await axios.put(`http://localhost:5001/department/${editingId}`, cleanData);
      } else {
        await axios.post("http://localhost:5001/department", cleanData);
      }
      fetchData();
      setIsModalOpen(false);
      setEditingId(null);
