    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.ldapClient.search(this.ldapBaseDn, searchOptions, (err, res) => {
          ...
          res.on('end', (result) => {
            if (!found) {
              console.log(`Searching for user with uid: ${username}`);
              reject(new Error('User not found'));
            }
          });
        });
  ...
  async getUserInfo(username) {
    try {
      ...
      return new Promise((resolve, reject) => {
        const searchOptions = {
          scope: 'sub',
          filter: `(mail=${username})`, // ตรวจสอบว่า filter นี้ถูกต้อง
        };

        this.ldapClient.search(this.ldapBaseDn, searchOptions, (err, res) => {
          if (err) {
            console.error('LDAP search failed:', err); // เพิ่ม log
            reject(new Error('LDAP search failed: ' + err.message));
            return;
          }
          ...
          res.on('error', (err) => {
            console.error('LDAP search error:', err); // เพิ่ม log
            reject(new Error('LDAP search error: ' + err.message));
          });

          res.on('end', () => {
            console.log('LDAP search result:', userData); // เพิ่ม log
            resolve(userData);
          });
        });
      });
    } catch (error) {
      console.error('Get user info error:', error); // เพิ่ม log
      return null;
    }
  }
